...
func main() {
	...
	man := NewMandelBrot(100, int(canvasSize.w), int(canvasSize.h))
	arr := man.MandelBrotFunc()

	//copy arr to image
	img_buf := GetImage(int(canvasSize.w), int(canvasSize.h), arr)
...
func setup() js.Value {
	...
	canvas = doc.Call("getElementById", "shader-container")
